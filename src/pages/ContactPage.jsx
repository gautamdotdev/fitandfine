...
function Field({ label, children }) {
  return (
    <label style={{ display: 'block' }}>
      <span className="label-caps" style={{ display: 'block', marginBottom: '8px' }}>{label}</span>
      {children}
    </label>
  );
}
  ...
  return (
    <div style={{
      ...
      <div>
        ...
        {lines.map((l) => (
          <p key={l} style={{ fontSize: '14px', color: 'var(--color-muted-foreground)', marginTop: '4px' }}>{l}</p>
        ))}
      </div>
    </div>
  );
...
export default function ContactPage() {
  ...
  return (
    <div className="page-transition" style={{ maxWidth: '1200px', margin: '0 auto', padding: '32px 20px 80px' }}>
      <div style={{ textAlign: 'center', maxWidth: '576px', margin: '0 auto' }}>
        <p className="label-caps" style={{ color: 'var(--color-gold)' }}>Contact</p>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: 'clamp(2.5rem, 5vw, 3rem)', marginTop: '12px' }}>Get in Touch</h1>
        <p style={{ color: 'var(--color-muted-foreground)', marginTop: '16px' }}>
          Questions about a piece, your order, or sizing — we'd love to help.
        </p>
      </div>

      <div style={{ marginTop: '56px', display: 'grid', gap: '40px' }} className="contact-grid">
        {/* Form */}
        <form
          onSubmit={(e) => {
            e.preventDefault();
            push({ type: 'success', message: "Message sent! We'll reply within 24 hours." });
        ...
        >
          <div style={{ display: 'grid', gap: '16px' }} className="form-name-email">
            <Field label="Name">
              <input
                required
                value={form.name}
          ...
          <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
            <button
              ...
              onMouseLeave={e => e.currentTarget.style.backgroundColor = 'var(--color-foreground)'}
            >
              Send Message
            </button>
            <a
              ...
              style={{
                ...
                fontSize: '14px', color: 'var(--color-whatsapp)', textDecoration: 'none',
              }}
        ...
        <aside style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
          ...
          <div style={{ aspectRatio: '4/3', borderRadius: '8px', overflow: 'hidden', border: '1px solid var(--color-border)' }}>
            <iframe
              title="Map"
