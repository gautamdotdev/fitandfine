...
const STYLES = `
  ...
  .uo-card { border: 1px solid var(--color-border); border-radius: 20px; overflow: hidden; background: var(--color-background); transition: box-shadow 0.2s; }
  ...
  .uo-card-meta { display: flex; align-items: center; gap: 14px; flex-wrap: wrap; }
            ...
            {STEPS.map((step, i) => {
                ...
                return (
                    <div key={step} className="uo-progress-step">
                        ...
                        />
                        <span className={`uo-progress-label ${done || current ? "active" : ""}`}>
                            {cfg.label}
                        </span>
                    </div>
                );
                                ...
                                <div key={i} className="uo-item">
                                    {img ? (
                                        ...
                                        <div className="uo-item-img-fallback">
                                            <ShoppingBag size={22} style={{ color: "var(--color-muted-foreground)" }} />
                                        </div>
                                    )}
                                    <div className="uo-item-info">
                                        <div className="uo-item-name">{product?.name || "Product"}</div>
                                    ...
                                    <div className="uo-item-price">
                                        ₹{((item.price || product?.price || 0) * (item.quantity || item.qty || 1)).toLocaleString("en-IN")}
                                    </div>
                                </div>
                            );
                        })}
