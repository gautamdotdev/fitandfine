import { products, categories, HERO_IMAGE, EDITORIAL_IMAGE, STORY_IMAGE } from './products.js';
...
export async function initializeImageCache() {
  ...
  try {
    const cacheName = 'veldra-image-cache-v1';
    ...
    for (let i = 0; i < ALL_IMAGES.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(url => 
          cache.add(url).catch(err => console.warn(`Could not cache image: ${url}`, err))
        )
      );
